    ...
    let _result = rayon::scope(move |s| {
        ...
        (0..num_miners).into_par_iter().for_each_with(tx, |tx, thread_id| {
            ...
            while !state.solution_found.load(Ordering::SeqCst) {
                ...
                // Format seed as hexadecimal (matching Java implementation)
                let seed_hex = format!("{:x}", current_seed);
                
                // Build complete message (reuse base_msg capacity)
                base_msg.truncate(base_len);
                base_msg.extend_from_slice(seed_hex.as_bytes());
                ...
                if count >= required_diff {
                    ...
                    if state.solution_found.compare_exchange(
                        ...
                        let seed_string = format!("{:x}", current_seed);  // Hexadecimal format
...
fn find_seed_gpu(
    ...
    __kernel void sha256_mine_kernel(
        ...
        // Convert seed to hexadecimal string (matching Java implementation)
        uchar seed_str[17];  // Max 16 hex digits for u64 + null
        ulong temp = seed;
        int seed_len = 0;
        const uchar hex_chars[] = "0123456789abcdef";
        ...
        if (temp == 0) {
            ...
            while (temp > 0 && digit_count < 16) {
                digits[digit_count++] = hex_chars[temp & 0xF];
                temp >>= 4;
            }
        ...
        // For bit counting, we need to work with the hash as bytes (big-endian)
        // Convert hash to byte array for proper bit counting
        uchar hash_bytes[32];
        for (int i = 0; i < 8; i++) {
            hash_bytes[i*4 + 0] = (uchar)(hash[i] >> 24);
            hash_bytes[i*4 + 1] = (uchar)(hash[i] >> 16);
            hash_bytes[i*4 + 2] = (uchar)(hash[i] >> 8);
            hash_bytes[i*4 + 3] = (uchar)(hash[i]);
        }
        
        // Count leading zero bits on byte array
        uint bits = 0;
        for (int i = 0; i < 32; i++) {
    ...
    for worker_id in 0..gpu_workers {
        ...
        let handle = thread::spawn(move || {
            ...
            while !state.solution_found.load(Ordering::SeqCst) {
                ...
                // DON'T reset best_bits - let it accumulate across batches
                // best_bits_host[0] = 0;  // Commented out to allow accumulation
                result_count_buf.write(&result_count_host).enq().unwrap();
                // Only write best_bits on first iteration
                if seed_base == (SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos() as u64 + (worker_id as u64 * 1000000)) {
                    best_bits_host[0] = 0;
                ...
                if kernel_duration.as_secs() > 5 {
                    eprintln!("Warning: Kernel took {:.2}s for {} hashes", kernel_duration.as_secs_f64(), batch_size);
                }
                ...
                // Debug: log every time we get a new best
                static LAST_REPORTED: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(0);
                if gpu_best > LAST_REPORTED.load(Ordering::Relaxed) && gpu_best > 0 {
                    eprintln!("DEBUG: GPU worker {} found new best: {} bits (batch had {} hashes)", 
                              worker_id, gpu_best, batch_size);
                    LAST_REPORTED.store(gpu_best, Ordering::Relaxed);
                }
                
                if gpu_best > 0 {  // Only update if we got a non-zero value
                    if gpu_best > best_overall.load(Ordering::SeqCst) {
                        best_overall.store(gpu_best, Ordering::SeqCst);
                    }
                    // Always update per-thread best to track this worker's progress
                    if let Ok(mut per) = best_per_thread.lock() {
                        if gpu_best > per[worker_id] {
                ...
                if count > 0 {
                    ...
                    for i in 0..count as usize {
                        ...
                        if bits >= required && required > 0 {
                            let seed_string = format!("{:x}", seed);  // Hexadecimal format
